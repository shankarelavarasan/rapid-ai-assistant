...
const SmoothScroll = ({ children, speed = 0.8, showScrollToTop = true, className = '' }) => {
    ...
    return (<>
      ...
      <div className="fixed top-0 left-0 w-full h-1 bg-black/10 z-50">
        <div className="h-full bg-gradient-to-r from-purple-500 via-pink-500 to-yellow-500 transition-all duration-300 ease-out" style={{ width: `${scrollProgress}%` }}/>
      </div>
      ...
      {/* Back to Top Button */}
      {showScrollToTop && showBackToTop && (<button onClick={scrollToTop} className="fixed bottom-8 right-8 z-50 p-3 bg-white/20 backdrop-blur-xl border border-white/30 text-white rounded-full shadow-lg hover:bg-white/30 hover:scale-110 transition-all duration-300 group" aria-label="Scroll to top">
...
export const ScrollReveal = ({ children, direction = 'up', delay = 0, duration = 0.6, threshold = 0.1, className = '' }) => {
    ...
    return (<div ref={elementRef} className={`transition-all ease-out ${className}`} style={{
...
export const ParallaxElement = ({ children, speed = 0.5, direction = 'up', className = '' }) => {
    const elementRef = useRef(null);
    ...
    return (<div ref={elementRef} className={`will-change-transform ${className}`}>
      {children}
    </div>);
};
// Smooth scroll navigation component
export const SmoothScrollNav = ({ sections, className = '' }) => {
    ...
    return (<nav className={`fixed left-8 top-1/2 -translate-y-1/2 z-40 ${className}`}>
      <div className="flex flex-col space-y-4">
        {sections.map((section) => (<button key={section.id} onClick={() => scrollToSection(section.id)} className={`group relative w-3 h-3 rounded-full transition-all duration-300 ${activeSection === section.id
                ? 'bg-white scale-125 shadow-lg'
                : 'bg-white/40 hover:bg-white/60 hover:scale-110'}`} aria-label={`Go to ${section.label}`}>
            {/* Tooltip */}
